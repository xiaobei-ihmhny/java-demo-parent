...
/**
 * A {@link java.util.concurrent.Flow.Publisher} that asynchronously issues submitted
 ...
 * java.util.concurrent.Flow.Subscription#request(long) request} do not directly result in
 ...
 * {@link java.util.concurrent.Flow.Subscriber#onError(Throwable) onError} and
 ...
 * @since 9
 */
public class SubmissionPublisher<T> implements java.util.concurrent.Flow.Publisher<T>,
                                               AutoCloseable {
    /*
    ...
    final BiConsumer<? super java.util.concurrent.Flow.Subscriber<? super T>, ? super Throwable> onNextHandler;
     ...
     * java.util.concurrent.Flow.Subscriber#onNext(Object) onNext}.
     *
    ...
    public SubmissionPublisher(Executor executor, int maxBufferCapacity,
                               BiConsumer<? super java.util.concurrent.Flow.Subscriber<? super T>, ? super Throwable> handler) {
        if (executor == null)
     ...
     * method {@link java.util.concurrent.Flow.Subscriber#onNext(Object) onNext}.
     *
     * @param executor the executor to use for async delivery,
     * supporting creation of at least one independent thread
     * @param maxBufferCapacity the maximum capacity for each
     ...
     * java.util.concurrent.Flow.Subscriber#onError(Throwable) onError} method is invoked on
     * the existing subscription with an {@link IllegalStateException}.
     * Otherwise, upon success, the Subscriber's {@link
     * java.util.concurrent.Flow.Subscriber#onSubscribe onSubscribe} method is invoked
     * asynchronously with a new {@link java.util.concurrent.Flow.Subscription}.  If {@link
     * java.util.concurrent.Flow.Subscriber#onSubscribe onSubscribe} throws an exception, the
     * subscription is cancelled. Otherwise, if this SubmissionPublisher
     * was closed exceptionally, then the subscriber's {@link
     * java.util.concurrent.Flow.Subscriber#onError onError} method is invoked with the
     ...
     * java.util.concurrent.Flow.Subscriber#onNext(Object) onNext} method. The item may be
     ...
     * java.util.concurrent.Flow.Subscriber#onError(Throwable) onError} signal to the
     * subscriber.
     *
    ...
    public int offer(T item,
                     BiPredicate<java.util.concurrent.Flow.Subscriber<? super T>, ? super T> onDrop) {
     ...
     * Publishes the given item, if possible, to each current subscriber
     * by asynchronously invoking its {@link
     * java.util.concurrent.Flow.Subscriber#onNext(Object) onNext} method, blocking while
     ...
     * error and/or issuing an {@link java.util.concurrent.Flow.Subscriber#onError(Throwable)
     ...
     * java.util.concurrent.Flow.Subscriber#onComplete() onComplete} signals to current
    ...
    /**
     * Unless already closed, issues {@link
     * java.util.concurrent.Flow.Subscriber#onError(Throwable) onError} signals to current
     ...
     * tracking purposes, not for invoking {@link java.util.concurrent.Flow.Subscriber}
     * methods on the subscribers.
     *
     * @return list of current subscribers
     */
    public List<java.util.concurrent.Flow.Subscriber<? super T>> getSubscribers() {
        ArrayList<java.util.concurrent.Flow.Subscriber<? super T>> subs = new ArrayList<>();
        synchronized (this) {
            BufferedSubscription<T> pred = null, next;
            for (BufferedSubscription<T> b = clients; b != null; b = next) {
                next = b.next;
     ...
     * @throws NullPointerException if subscriber is null
     */
    public boolean isSubscribed(java.util.concurrent.Flow.Subscriber<? super T> subscriber) {
        if (subscriber == null) throw new NullPointerException();
        if (!closed) {
            synchronized (this) {
                BufferedSubscription<T> pred = null, next;
     ...
     * publisher signals {@link java.util.concurrent.Flow.Subscriber#onComplete()
...
//    @jdk.internal.vm.annotation.Contended
    private static final class BufferedSubscription<T>
        implements java.util.concurrent.Flow.Subscription, ForkJoinPool.ManagedBlocker {
        ...
        // 这里包含了我们要传入的订阅者的信息
        java.util.concurrent.Flow.Subscriber<? super T> subscriber; // null if disabled
        Executor executor;                 // null if disabled
        BiConsumer<? super java.util.concurrent.Flow.Subscriber<? super T>, ? super Throwable> onNextHandler;
        ...
        // 这里通过next来构造 Publisher 的执行链，也就是一堆订阅者在此做一个编排
         ...
         *
         * <p>当 maxBufferCapacity 大于这个值时，使用默认的初始大小
         * maxBufferCapacity的大小必须为2的n次方</p>
         */
        static final int DEFAULT_INITIAL_CAP = 32;

        BufferedSubscription(java.util.concurrent.Flow.Subscriber<? super T> subscriber,
                             Executor executor,
                             BiConsumer<? super java.util.concurrent.Flow.Subscriber<? super T>,
...
//                ForkJoinPool.helpAsyncBlocker(executor, this);
        ...
        final void onError(Throwable ex) {
            for (int c;;) {
                if (((c = ctl) & (ERROR | DISABLED)) != 0)
                    ...
                    java.util.concurrent.Flow.Subscriber<? super T> s = subscriber;
                    if (s != null && ex != null) {
                        try {
                            s.onError(ex);
         ...
         */
        final void consume() {
            java.util.concurrent.Flow.Subscriber<? super T> s;
            int h = head;
         ...
         */
        private boolean checkControl(java.util.concurrent.Flow.Subscriber<? super T> s, int c) {
            boolean stat = true;
            if ((c & SUBSCRIBE) != 0) {
         ...
         */
        private boolean checkEmpty(java.util.concurrent.Flow.Subscriber<? super T> s, int c) {
            boolean stat = true;
            if (head == tail) {
                if ((c & CONSUME) != 0)
                    CTL.compareAndSet(this, c, c & ~CONSUME);
         ...
         */
        private void handleOnNext(java.util.concurrent.Flow.Subscriber<? super T> s, Throwable ex) {
