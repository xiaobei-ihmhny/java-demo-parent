 ...
 */
@DisplayName("示例 java8 stream 基本用法")
public class StreamTest {
    ...
    void foreachFindMatch() {
        ...
        // 匹配第一个年龄大于18岁的员工元素，若存在时输出员工信息
        Optional<Person> firstPerson = personList.stream().filter(p -> p.getAge() > 18).findFirst();
        firstPerson.ifPresent(p -> System.out.printf("匹配的第一个元素为：%s%n", p));
        // 找出任何一个性别为男性的员工，若存在时输出员工信息
        Optional<Person> findPerson = personList.parallelStream().filter(p -> "male".equals(p.getSex())).findAny();
        findPerson.ifPresent(p -> System.out.printf("匹配的一个元素为：%s%n", p));
        // 判断是否存在薪资高于9000员工
        boolean anyMatch = personList.stream().anyMatch(p -> p.getSalary() > 9000);
     ...
     */
    @Test
    @DisplayName("max/min/count 用法示例")
    void maxMinCount() {
        // TODO 1. 获取 String 集合中最长的元素 ["a", "b", "c", "dd11", "ee"]
        ...
        long above8000Count = personList.stream().filter(p -> p.getSalary() > 8000).count();
    ...
    void mapFlatMap() {
        ...
        // 将员工的薪资全部增加1000。
        System.out.printf("调薪前的薪资为：%s%n", personList);
    ...
    class Collect {
        ...
        void toCollection() {
            ...
            Map<String, Person> resultMap = personList.stream()
                    .filter(p -> p.getSalary() > 8000)
        ...
        void countAveraging() {
            // 员工中薪资高于8000的人数有多少？
            Long count = personList.stream().filter(p -> p.getSalary() > 8000).collect(Collectors.counting());
        ...
        void joining() {
            ...
            String nameJoined = personList.stream()
                    .map(Person::getName)
        ...
        void reducing() {
            // 每个员工减去起征点后的薪资之和
            personList.stream().collect(Collectors.reducing(0, p -> p.getSalary() - 1500, (i, j) -> i + j - 5000));
            // stream的reduce
        }
        ...
        void sorted() {
            ...
            // 先按工资再按年龄升序排序
            List<Person> sortedBySalaryAndAgeList = personList.stream()
                    .sorted(Comparator.comparing(Person::getSalary)
                            .thenComparing(Person::getAge))
                    .collect(Collectors.toList());
            System.out.println("先按工资再按年龄升序排序：" + sortedBySalaryAndAgeList);
            // 先按工资升序再按年龄降序排序
            List<Person> customSortedList = personList.stream()
                    .sorted((p1, p2) -> {
