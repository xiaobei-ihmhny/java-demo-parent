...
public class ThreadPoolExecutor extends AbstractExecutorService {
     ...
     * runWorker).
     *
     * <p>{@link Worker}类主要维护线程运行任务的中断控制状态，以及其他次要簿记。
     ...
     * 另外，为了抑制直到线程真正开始运行任务之前的中断，
     * 我们将锁定状态初始化为负值，并在启动时将其清除（在runWorker中）。
     *
     *
     * <p>通过Worker的定义可以看到，每个Worker对象都有一个Thread线程对象与它相对应，
     * 当任务需要执行的时候，实际是调用内部Thread对象的start方法，
     * 而Thread对象是在Worker的构造器中通过getThreadFactory().newThread(this)方法创建的，
     * 创建的Thread将Worker自身作为任务，所以当调用Thread的start方法时，
     * 最终实际是调用了Worker.run()方法，该方法内部委托给runWorker方法执行任务
     *
     * <p>Worker表示线程池中的一个工作线程, 可以与任务相关联.
        ...
        try {
            ...
            if (t != null) {
                ...
                try {
                    ...
                    // 获取锁之后重新检查，如果ThreadFactory失败或在获得锁之前关闭，请回退。
                    int rs = runStateOf(ctl.get());
